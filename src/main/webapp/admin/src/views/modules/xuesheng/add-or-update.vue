		...
		>
			<template >
				...
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' class="select" v-if="type!='info'"  label="社团类型" prop="shetuanleixing">
					<el-select :disabled="ro.shetuanleixing" v-model="ruleForm.shetuanleixing" placeholder="请选择社团类型" multiple>
						<el-option
...
export default {
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.shetuanleixing = this.ruleForm.shetuanleixing.split(",");
        } else {
          this.$message.error(data.msg);
        }
      });
       ...
       this.$refs["ruleForm"].validate(valid => {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
             ...
             this.ruleForm.shetuanleixing = this.ruleForm.shetuanleixing.join(",");
			 this.$http({
			   url: `xuesheng/${!this.ruleForm.id ? "save" : "update"}`,
			   method: "post",
			   data: this.ruleForm
