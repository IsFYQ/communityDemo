...
<script>
  export default {
    ...
    //方法集合
    methods: {
      // 调用forumController中的list方法，将本条帖子的id作为值传给后端
      getCommentList() {
        this.$http.get(`forum/list/${this.detail.id}`).then(res => {
          if (res.data.code == 0) {
            // 将forumController中的childs方法返回的值赋给commentList
      ...
      addForum(formName) {
        let sensitiveWords = "";
        let sensitiveWordsArr = ['妈的','艹','草泥马','操你妈','shit','龟儿子','龟孙','公安局','政府','办证','人流','贷款','国家机关','乱伦类','迷奸','摄像头','手枪','窃听器','安眠药'];
        if(sensitiveWords) {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            // 将这个表单中的数据传回服务器，调用forum中的add方法，之后会收到服务器传回来的信息，接收此信息
            this.$http.post('forum/add', this.form).then(res => {
              // 如果正常执行，返回的code为0，给出信息：评论成功，然后返回评论列表
              if (res.data.code == 0) {
                this.$message({
                  type: 'success',
                  message: '评论成功!',
