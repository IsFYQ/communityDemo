  ...
  export default {
    ...
    // 多级联动参数
      ...
      onSubmit() {
        ...
        var crossrefid;
        var crossoptnum;
        // 最开始就已经声明了ref=ruleForm，在这里就可以用$refs["ruleForm"]来获取这个表单对象
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            if(this.type=='cross'){
              // 检查本地存储中的 "statusColumnName" 和 "statusColumnValue" 值是否存在（且不为空）
                 var statusColumnName = localStorage.getItem('statusColumnName');
                 var statusColumnValue = localStorage.getItem('statusColumnValue');
                 if(statusColumnName && statusColumnName!='') {
                     var obj = JSON.parse(localStorage.getItem('crossObj'));
                   // 如果 "statusColumnName" 不以 "[" 开头，则它会遍历名为 "obj" 的 JSON 对象中所有键名，
                   // 并找到与 "statusColumnName" 相同的项，然后将相应项的值设置为 "statusColumnValue"，
                   // 最后将 "obj" 对象传递给指定表格的 "/update" 接口进行更新。
                   ...
                   // 如果 "statusColumnName" 以 "[" 开头，
                     // 则它会分别从本地存储中读取 "userid"、"crossObj"、"crossTable" 和 "statusColumnName" 的值，
                     // 并替换掉 "[" 和 "]" 字符
                     else {
