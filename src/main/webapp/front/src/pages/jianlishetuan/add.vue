...
<div :style='{"width":"100%","padding":"40px 7%","margin":"0 auto","position":"relative","background":"none"}'>
    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px 0","background":"none"}' label="社团人数" prop="shetuanrenshu">
            <el-input v-model="ruleForm.shetuanrenshu" 
                placeholder="社团人数" clearable ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px 0","background":"none"}'  label="社团类型" prop="shetuanleixing">
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px 0","background":"none"}' label="负责内容" prop="fuzeneirong">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="负责内容"
...
<script>
  export default {
      ...
      init(type) {
        ...
        this.$http.get(this.userTableName + '/session', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            ...
            // 判断 json 对象中的 xueshengzhanghao 和 xueshengxingming 属性是否存在
            // 如果存在，就将其赋值给 ruleForm 对象的 xueshengzhanghao 和 xueshengxingming 属性
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('jianlishetuan/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('jianlishetuan/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          this.$http.post('jianlishetuan/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
