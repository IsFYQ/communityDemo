	...
	<div class="addEdit-block" :style='{"padding":"30px 30px 0"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' class="textarea" v-if="type!='info'" label="解散原因" prop="jiesanyuanyin">
					<el-input
					  style="min-width: 200px; max-width: 600px;"
					  type="textarea"
					  :rows="8"
					  placeholder="解散原因"
					  v-model="ruleForm.jiesanyuanyin" >
					</el-input>
				</el-form-item>
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' v-else-if="ruleForm.jiesanyuanyin" label="解散原因" prop="jiesanyuanyin">
					<span :style='{"fontSize":"14px","lineHeight":"40px","color":"rgba(90, 67, 74, 1)","fontWeight":"500","display":"inline-block"}'>{{ruleForm.jiesanyuanyin}}</span>
				</el-form-item>
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						...
						if(o=='jiesanyuanyin'){
							this.ruleForm.jiesanyuanyin = obj[o];
							this.ro.jiesanyuanyin = true;
							continue;
						}
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jiesanshetuan/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jiesanshetuanCrossAddOrUpdateFlag = false;
						 this.parent.search();
