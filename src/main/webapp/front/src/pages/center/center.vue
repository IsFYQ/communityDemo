    ...
    <el-tabs tab-position="left" :style='{"background":"none"}' @tab-click="handleClick">
      <el-tab-pane label="个人中心">
        <el-form class="center-preview-pv" ref="sessionForm" :model="sessionForm" :rules="rules" label-width="150px">
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' v-if="userTableName=='xuesheng'" label="学生账号" prop="xueshengzhanghao">
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' v-if="userTableName=='xuesheng'" label="头像">
            <el-upload
              ...
              <img :style='{"border":"1px dashed #ccc","cursor":"pointer","color":"#555","borderRadius":"6px","textAlign":"center","width":"200px","fontSize":"32px","lineHeight":"200px","height":"200px"}' v-if="sessionForm.touxiang" :src="baseUrl + sessionForm.touxiang" class="avatar">
              <i v-else class="el-icon-plus avatar-uploader-icon"></i>
            </el-upload>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' v-if="userTableName=='xuesheng'" label="年龄" prop="nianling">
            <el-input v-model="sessionForm.nianling" placeholder="年龄" ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' v-if="userTableName=='xuesheng'" label="电话号码" prop="dianhuahaoma">
            <el-input v-model="sessionForm.dianhuahaoma" placeholder="电话号码" ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","background":"none"}' v-if="userTableName=='xuesheng'" label="社团类型">
            <el-select v-model="sessionForm.shetuanleixingArray" placeholder="请选择社团类型" multiple @change="xueshengshetuanleixingMulChange" >
              <el-option v-for="(item, index) in dynamicProp.shetuanleixing" :key="index" :label="item" :value="item"></el-option>
            </el-select>
          </el-form-item>
...
<script>
  ...
  export default {
    ...
    created() {
      ...
      if ('xuesheng' == this.userTableName) {
        this.$set(this.sessionForm, 'shetuanleixing', null);
      }
      ...
      init() {
        ...
        if ('xuesheng' == this.userTableName) {
          this.$http.get('option/shetuanleixing/shetuanleixing', {emulateJSON: true}).then(res => {
            if (res.data.code == 0) {
              this.dynamicProp.shetuanleixing = res.data.data;
            }
          });
        }
      },
        xueshengshetuanleixingMulChange(e){
            this.sessionForm.shetuanleixing = this.sessionForm.shetuanleixingArray.join(',')
        },
      getSession() {
        this.$http.get(this.userTableName + '/session', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
            if(res.data.data.shetuanleixing){
                res.data.data.shetuanleixingArray = res.data.data.shetuanleixing.split(',')
            }else{
                res.data.data.shetuanleixingArray = []
            }
